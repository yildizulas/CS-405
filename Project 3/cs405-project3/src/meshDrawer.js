...
void main()
{
	...
	if (facingLight > 0.0) {
		...
		vec3 viewPos = vec3(0.0, 0.0, -1.0); // Camera position
		vec3 viewdir = normalize(viewPos - fragPos); // Direction from the fragment to the camera
		vec3 reflectdir = reflect(-lightdir, normal); // Reflection vector
		spec = pow(max(dot(viewdir, reflectdir), 0.0), phongExp) * facingLight;
	} else {
		// Set both diffuse and specular components to zero for non-illuminated surfaces
		diff = 0.0;
		spec = 0.0;
	}
